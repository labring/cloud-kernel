...
import (
	...
	"github.com/cuisongliu/cloud-kernel/pkg/dingding"
...
func ProcessError(err error) {
	...
	case *errors.ServerError:
		...
		dingding.DingdingLink("离线包打包失败", "创建虚拟机step1失败,错误码:"+e.ErrorCode()+",详细信息: "+e.Message(), e.Recommend(), false)
	case *errors.ClientError:
		e := err.(*errors.ClientError)
		logger.Error("SDK.ClientError")
		logger.Error("ErrorCode: ", e.ErrorCode())
		logger.Error("Message: ", e.Message())
		dingding.DingdingText("离线包打包失败,创建虚拟机step1失败,错误码:"+e.ErrorCode()+",详细信息: "+e.Message(), false)
	}
	//_ = os.Stderr.Close()
	os.Exit(0)
}
